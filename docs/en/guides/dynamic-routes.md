...
export const GET = (c: Context) => {
	...
	if (!isValidSlug(slug)) {
		return c.json(
			{
				error:
					'Invalid slug format. Must be 3+ characters, alphanumeric with hyphens/underscores.',
			},
			400
		)
	}
	...
	return c.json({
		...
		breadcrumb: [
			...
			{ name: 'Projects', path: `/organizations/${params.orgId}/projects` },
			{
				...
				path: `/organizations/${params.orgId}/projects/${params.projectId}`,
			},
			{
				name: 'Tasks',
				path: `/organizations/${params.orgId}/projects/${params.projectId}/tasks`,
			},
			{
				name: params.taskId,
				path: `/organizations/${params.orgId}/projects/${params.projectId}/tasks/${params.taskId}`,
			},
		],
	})
}
...
export const GET = async (c: Context) => {
	...
	try {
		...
		return c.json({ error: 'Failed to fetch from external API' }, 500)
	}
}
...
export const POST = async (c: Context) => {
	...
	if (!segments[0] || !postAllowed.includes(segments[0])) {
		return c.json({ error: 'POST not allowed for this endpoint' }, 405)
	}
...
export const validateParams = (
	...
	schema: Record<string, keyof typeof validators>
) => {
...
1. Learn about [Advanced Features](/guides/advanced-features) for middleware and hooks
2. Explore the [API Reference](/reference/api) for complete documentation
3. Check out [Examples](/examples/basic) for real-world implementations
