...
export const GET = async (c: Context) => {
	try {
		...
		return c.json(createErrorResponse('INTERNAL_ERROR', '发生了意外错误'), 500)
	}
}
```
...
// 在路由中使用
export const GET = async (c: Context) => {
	const users = await withCache(() => userService.getAllUsers(), {
		ttl: 600,
		key: 'all-users',
	})
...
export function withRateLimit(requests: number, windowMs: number) {
	...
	return async (c: Context, next: () => Promise<void>) => {
		...
		if (recentRequests.length >= requests) {
			return c.json(
				createErrorResponse(
					new APIError(ErrorCodes.SYS_RATE_LIMIT_EXCEEDED, '请求频率超限', 429)
				),
				429
			)
		}
...
- [性能指南](/zh/guides/performance.md) - 高级优化技术
- [部署指南](/zh/guides/deploy.md) - 生产部署策略
- [API 参考](/zh/reference/api.md) - 完整 API 文档
- [项目示例](/zh/examples/projects.md) - 真实世界的应用示例
