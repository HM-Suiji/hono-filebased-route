...
export const useDatabase = (c: Context) => {
	...
	return {
		...
		delete: async (table: string, id: string | number): Promise<boolean> => {
			const results = await db.query(`DELETE FROM ${table} WHERE id = $1`, [id])
...
export const registerDynamicRoutes = (app: Hono, routes: DynamicRoute[]) => {
	routes.forEach(({ path, method, handler, middleware = [] }) => {
		const routeHandler =
			middleware.length > 0 ? [...middleware, handler] : handler
...
export const monitoringMiddleware = async (c: Context, next: Next) => {
	...
	if (duration > 1000) {
		console.warn(
			`Slow request: ${metric.method} ${metric.path} took ${duration}ms`
		)
	}
}
...
export interface AppContext extends Context {
	...
	set<T extends keyof ContextVariableMap>(
		key: T,
		value: ContextVariableMap[T]
	): void
}
```
...
- [API 参考](/zh/reference/api.md) - 详细的 API 文档
- [配置选项](/zh/reference/configuration.md) - 所有配置选项
- [示例项目](/zh/examples/projects.md) - 完整的示例项目
- [部署指南](/zh/guides/deploy.md) - 生产环境部署
- [故障排除](/zh/guides/troubleshooting.md) - 常见问题解决方案
