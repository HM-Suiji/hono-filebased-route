...
class MemoryMonitor {
	...
	private checkMemoryUsage() {
		...
		console.log(
			`内存使用: ${heapUsedMB.toFixed(2)}MB / ${heapTotalMB.toFixed(2)}MB (${(
				heapUsageRatio * 100
			).toFixed(1)}%)`
		)
...
class LoadTester {
	...
	async runTest() {
		console.log(
			`🚀 开始负载测试，${this.concurrentUsers}个并发用户，持续${
				this.duration / 1000
			}秒`
		)
	...
	generateReport() {
		...
		Object.entries(endpointStats)
			...
			.forEach(([endpoint, stats]) => {
				console.log(
					`${endpoint}: ${stats.avgDuration.toFixed(2)}ms 平均 (${
						stats.count
					} 请求)`
				)
			})
...
- [部署指南](/zh/guides/deploy.md) - 生产环境部署策略
- [最佳实践](/zh/examples/best-practices.md) - 代码组织和模式
