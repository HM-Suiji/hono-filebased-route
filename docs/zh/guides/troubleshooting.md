...
1. **添加响应时间日志**：
   ```typescript
   // 跟踪响应时间的中间件
   export const responseTimeMiddleware = async (c: Context, next: () => Promise<void>) => {
     ...
     if (duration > 1000) { // 记录慢请求
       console.warn(`慢请求: ${c.req.method} ${c.req.url} 耗时 ${duration.toFixed(2)}ms`)
     }
   }
   ```
...
2. **请求跟踪中间件**：
   ...
   export const requestTracing = async (c: Context, next: () => Promise<void>) => {
...
1. **查看文档**：查阅 [API 参考](../reference/api.md) 和 [示例](../examples/)
...
- [最佳实践](../examples/best-practices.md) - 代码组织模式
